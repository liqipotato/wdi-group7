...
public class FoundingdateFuserShortestString extends AttributeValueFuser<String, Company, Attribute> {
	...
	public void fuse(RecordGroup<Company, Attribute> group, Company fusedRecord,
		...
		fusedRecord.setFoundingDate(fused.getValue());

		// add provenance info
		fusedRecord.setAttributeProvenance(Company.FOUNDINGDATE, fused.getOriginalIds());
	}

	@Override
	public boolean hasValue(Company record, Correspondence<Attribute, Matchable> correspondence) {
		return record.hasValue(Company.FOUNDINGDATE);
	}
