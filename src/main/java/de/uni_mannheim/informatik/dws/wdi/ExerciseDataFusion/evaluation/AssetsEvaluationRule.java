...
public class AssetsEvaluationRule extends EvaluationRule<Company, Attribute> {
	...
	public boolean isEqual(Company record1, Company record2, Attribute schemaElement) {
		if (Double.toString(record1.getAssets()) == null && Double.toString(record2.getAssets()) == null)
			return true;
		else if (Double.toString(record1.getAssets()) == null ^ Double.toString(record2.getAssets()) == null)
			return false;
		else
			return Double.toString(record1.getAssets()).equals(Double.toString(record2.getAssets()));
	}
